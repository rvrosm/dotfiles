...
"runtimepath = $HOME .. '/.vim/myplugin:' .. runtimepath
...
" gopass {{{
if has("mac")
  autocmd BufNewFile,BufRead /private/**/gopass** setlocal noswapfile nobackup noundofile shada="" viminfo=""
else
...
let g:rg_command = 'rg --column --line-number --no-heading --fixed-strings --ignore-case --no-ignore --hidden --follow --color "always" -g "*.{js,json,php,md,styl,jade,html,config,py,cpp,c,go,hs,rb,conf}" -g "!{.git,node_modules,vendor}/*,tags,.DS_Store"'

function! Rg(args, bang)
  call fzf#vim#grep(
    'rg --column --line-number --no-heading --color=always ' .. shellescape(a:args),
    1,
    a:bang == 0 ? fzf#vim#with_preview('right:50%:hidden', '?')
               : fzf#vim#with_preview('up:60%'),
    a:bang
  )
endfunction

function! F(args, bang)
  call fzf#vim#grep(g:rg_command .. shellescape(a:args), 1, a:bang)
endfunction
