...
# gopass {{{
if has("mac")
    autocmd BufNewFile,BufRead /private/**/gopass** :setlocal noswapfile nobackup noundofile viminfo='NONE'
else
    autocmd BufNewFile,BufRead /dev/shm/gopass* :setlocal noswapfile nobackup noundofile viminfo='NONE'
endif
# gopass }}}
...
# inoremap <expr> <cr> "\<Plug>(vimcomplete-do-complete)" ?? "<cr>"
...
g:rg_command = 'rg --column --line-number --no-heading --fixed-strings --ignore-case --no-ignore --hidden --follow --color "always" -g "*.{js,json,php,md,styl,jade,html,config,py,cpp,c,go,hs,rb,conf}" -g "!{.git,node_modules,vendor}/*,tags,.DS_Store"'

def Rg(args: string, bang: number)
  fzf#vim#grep(
    'rg --column --line-number --no-heading --color=always ' .. shellescape(args),
    1,
...
def F(args: string, bang: number)
  fzf#vim#grep(g:rg_command .. shellescape(args), 1, bang)
enddef
...
# python-syntax }}}
# debug vimspector {{{
g:vimspector_bottombar_height = 5
